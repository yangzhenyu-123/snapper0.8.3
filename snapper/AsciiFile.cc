...
namespace snapper
{
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "( fd =" << fd << ")");
	if (file == NULL)
	{
	    y2war("file is NULL");
    ...
    {
		 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(FILE* )");
	if (file == NULL)
	{
	    y2war("file is NULL");
	    SN_THROW(FileNotFoundException());
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(filename=" << filename << ")");
      ...
      remove_empty(remove_empty)
{
    ...
    AsciiFile::reload()
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(loading "  << Name_C << ")");
	...
	string line;
	y2err("now it is in  snapper/AscillFile.cc" << __FILE__ << "|| func name: getline() ");
...
AsciiFile::save()
{
	...
	y2mil("deleting file " << Name_C);
	 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "() || deleting file " << Name_C);
	...
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "() || saving file " << Name_C);
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__  << "(key="<< key << ", bool value=" << value << ")");	
	...
	value = tmp == "yes";
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(key=" << key << ", const bool value=" << value << ")");
	return true;
    }
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__  << "(key="<< key << ", char value=" << value << ")");
	...
	if (it == lines().end())
	    ...
	    *it = line;
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__  << "(key="<< key << ", string value=" << value << ")");
	modified = true;
    }
	...
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(key="<< key << ", const string value=" << value << ")");
	return true;
    }
    ...
    {
		 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__  << "(key="<< key << ", values)");
	string tmp;
	...
	if (!buffer.empty())
	    values.push_back(buffer);
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__  << "(key=" << key << ", values.size=" << values.size() <<")");
	return true;
    }
