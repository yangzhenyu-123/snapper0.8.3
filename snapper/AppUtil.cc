...
namespace snapper
{
    ...
    {
		//y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(path_cv=" << Path_Cv << ")");
	struct stat Stat_ri;
    ...
    {
 	y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "src=" << src_fd << ", dest=" << dest_fd << ")");
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "path=" << path << ", buf=" << buf << ")");
	char tmp[1024];
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "oldpath=" << oldpath << ", newpath=" << newpath << ")");
    ...
    {
	//	y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(" << path << ")");
        if (root_prefix == "/")
            return path;
        else
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(" << name << ")");
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(mount_point=" << mount_point <<  ")");
	...
	if (strftime(buf, sizeof(buf), classic ? "%F %T" : "%c", &t2) == 0)
		...
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(t1=" << string(buf) << ", utc=" << utc << ", classic=" << classic << ")");
	return string(buf);
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(uid=" << uid << ", username=" << username << ", gid=" << gid << ")");
	struct passwd pwd;
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(username=" << username << ", uid=" << uid << ")");
	struct passwd pwd;
	struct passwd* result;
    ...
    {
		y2err("now it is in  snapper/" << __FILE__ << "|| func name: " << __FUNCTION__ << "(group=" << groupname << ", gid=" << gid << ")");
	struct group grp;
	struct group* result;
