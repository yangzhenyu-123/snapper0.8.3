...
namespace snapper
{
    ...
    cmpFiles(const SFile& file1, const struct stat& stat1, const SFile& file2,
	     const struct stat& stat2)
    {
		 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "("<< file1.fullname() << ", stat1=" << stat1.st_mode << file2.fullname() << ", stat2=" << stat2.st_mode << ")");
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(" << file1.fullname()<< ", " << file2.fullname() << ")");
	struct stat stat1;
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(dir, path:" << path << ", status:" << status << ")");
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(dir=" 
	<< dir.fullname() << ", path:" << path << ", name:" << name << ", stat" << ", status:" << status <<", cb)");
	cb(path + "/" + name, status);
    ...
    {
		 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(cmp_data , dir1:" << 
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ <<
	 "(cmp_data.dev1=" << cmp_data.dev1<<", cmp_data.dev2=" << cmp_data.dev2 << ", dir1:"
	  << dir1.fullname() << ", dir2:" << dir2.fullname() << ", " << path << ")" );
	boost::this_thread::interruption_point();

	vector<string> entries1 = dir1.entries();
	y2err("调用sort对dir1中的文件进行排序");
	...
	y2err("打印dir1和 dir2");
	cout << "dir1: ";
	for (auto i = 0; i != entries1.size();++i)
	cout << entries1[i] << " ";

	cout << "\n";

	cout << "dir2: ";
	for (auto j = 0; j != entries2.size();++j)
	cout << entries2[j] << "  ";
	...
	while (first1 != last1 || first2 != last2)
	{
	    ...
	    else if (first1 == last1) //dir1只有一个
	    {
		cout << "first1 == last1 =" << *first1 << endl;
		struct stat stat2;
		dir2.stat(*first2, &stat2, AT_SYMLINK_NOFOLLOW); // TODO error check

		if (stat2.st_dev == cmp_data.dev2)
		{
			y2err("当匹配后，调用lonesome函数 CREATED 也就是dir1没有 dir2中有");
			lonesome(dir2, path, *first2, stat2, CREATED, cmp_data.cb);
		}
		++first2;
	    }
	    else if (first2 == last2) //dir2只有一个
	    {
		cout << "first2 == last2 =" << *first2 << endl;	
		struct stat stat1;
	    ...
	    {
		cout << "*first2 < *first1 ：first2=" << *first2 << ", first1=" << *first1 << endl;
		struct stat stat2;
		dir2.stat(*first2, &stat2, AT_SYMLINK_NOFOLLOW); // TODO error check
	    ...
	    {
		cout << "*first1 < *first2 ：first2=" << *first2 << ", first1=" << *first1 << endl;
		struct stat stat1;
		dir1.stat(*first1, &stat1, AT_SYMLINK_NOFOLLOW); // TODO error check
    ...
    {
	y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(dir1:" << dir1.fullname() << ", dir2:" << dir2.fullname() << ", cb)");
    ...
    {
		 y2err("now it is in  snapper/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(file1, file2)");
        try
        {
