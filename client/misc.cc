...
{
	 y2err("now it is in  client/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(" << str << ")");
    istringstream s(str);
    unsigned int num = 0;
    s >> num;
...
{
	 y2err("now it is in  client/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(map<> userdata)");
    string s;
...
username(uid_t uid)
{
	 y2err("now it is in  client/" << __FILE__ << "|| func name:" << __FUNCTION__ << "(uid=" << uid << ")");
    string username;
    gid_t gid;
    ...
    : command(DIFFBIN " --new-file --unified"), extensions()
{
	...
	y2err("实际调用的函数是 diff --new-file --unified");
}
...
{
	 y2err("now it is in client/" << __FILE__  << " ||| func name is:" << __FUNCTION__ << "(f1=" << f1 << ", f2=" << f2 << ")");
    string tmp = command;
    if (!extensions.empty())
	tmp += " " + extensions;
